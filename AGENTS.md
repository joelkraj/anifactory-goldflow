...
- Scene reference attachment prioritizes visible character refs first, then location, props, style, UI, and action/effects. If a visible named character already has an approved character_state_ref for that scene, imagegen may attach it even when the prompt planner omitted it. Do not attach multiple state refs for the same character in one cut.
