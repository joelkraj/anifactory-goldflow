...
- Score should be layered, not only windowed. Chapter beds are the base emotional floor; a future score-drop layer should place short Whisper-timed musical accents on focal drama, hype, reversal, reveal, and payoff beats. Score drops must remain separate from physical SFX, duck under narration, and be used sparingly enough that they feel intentional.
...
- Scene reference attachment prioritizes visible character refs first, then location, then prop/UI, then action/effects. Priority kind outranks required/optional flags, so a visible character ref is not displaced by a required lower-priority ref. Style refs are only attached to scene cuts when no concrete refs are available, because character, location, prop, and UI refs already carry style. If a visible named character already has an approved character_state_ref for that scene, imagegen may attach it even when the prompt planner omitted it. Do not attach multiple state refs for the same character in one cut.
...
- Score generation: prefer local ACE-Step 1.5 for chapter score beds; current default local model pair is `acestep-v15-turbo` plus `acestep-5Hz-lm-1.7B`. Planned upgrade: add a separate Whisper-timed score-drop layer for drama, hype, reveal, and payoff beats.
