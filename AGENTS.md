...
- Image prompts that attach references must include positive Flux-style reference slot mapping in prompt text, such as "Use image one as character identity for Kang Jiwoo; use image two as the dungeon location; use image three as the blue attention-thread effect." Attachment order must be intentional, not incidental. If the imagegen wrapper injects this slot mapping automatically, do not duplicate the same "Use image..." instructions inside the authored scene prompt body.
- Scene prompt bodies should request one continuous full-frame image with the current scene environment filling the frame. Reference images guide identity, wardrobe, style, UI, props, and effects; they should not be described as visible panels, sheets, or background content inside the final cut.
