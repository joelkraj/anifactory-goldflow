...
- Image provider routing is explicit. If the operator says "run it through the pipeline normally" or does not name an image provider, use the goldflow default image lane: ModelsLab Flux Klein for references and scene image cuts, with Codex/local-Qwen used only for LLM planning/review tasks. If the operator says "Codex Imagen", "Codex imagegen", "use Codex for images", or similar, do not run `scripts/imagegen.mjs`'s default ModelsLab route for scene cuts. Use the Codex/OpenAI image-generation lane instead, or stop and wire/migrate that lane before spending image budget.
- Codex image generation capability exists in the older AniFactory stack, not as the current goldflow `imagegen start` default. Legacy reference points: `/Users/joel/anifactory/lib/providers/codex-imagegen.ts`, `/Users/joel/anifactory/config/codex-imagegen.json`, and archived routes under `/Users/joel/anifactory/archive/legacy-api-routes/app/api/providers/codex-imagegen/`. That provider calls `codex exec --enable image_generation`, can attach refs with `--image`, imports generated rasters from `CODEX_HOME/generated_images`, and uses config mode `approved_batch`. For goldflow, treat this as an available route to migrate/use intentionally when requested, not as interchangeable with ModelsLab.
...
- Render: one continuous mixed audio track, final-script subtitle text timed by Whisper, yellow subtitle text with a small black outline and no background box. Ken Burns motion is full-frame, profile-based, visible, and intentional by default: hook bursts, action pushes, reveal pushes, wide drifts, emotional holds, and steady pushes. Use blurred foreground drift only as a deliberate override. Transitions should be hand-picked by beat; do not default every scene change to fade-to-black. Use smooth FFmpeg transitions such as wipes, pushes, slides, flashes, and manga-panel swipes where they match the scene energy, with transition SFX bound to the actual transition on selected cuts, especially throughout the opening thirty seconds. Longform renders should use bounded clip concurrency and a moderate Ken Burns overscale multiplier so motion stays visible without serial 4K intermediates becoming the bottleneck; use `--motion fill_pan` for long still-image episodes when `zoompan` is too slow. Extract final QA frames and spot-check prompt/image/ref consistency before treating the render as publishable.
