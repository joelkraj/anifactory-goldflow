...
23. Directed motion and render.
   ...
   - Premium motion uses stillness as contrast: true static holds for already-readable reactions/tension/UI, one restrained move for ordinary directed cuts, and optional layered 2.5D treatment only for a few image-QA-inspected hero frames. For a bounded proof, build layers with `goldflow visual parallax-assets`, inspect the generated mask and high-resolution start/mid/end frames, then bind the layer hashes into the proof motion plan. Blurred background plates must obey the `foreground_cover` contract: both layers share keyframe anchors/timing/easing and foreground scale stays at or above background scale. Never translate a cutout laterally over an incompletely inpainted plate; fall back to single-plane motion or a static hold.
