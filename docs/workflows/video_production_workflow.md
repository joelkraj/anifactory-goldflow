...
14. Visual beat planning.
   ...
   - Semantic scenes are broad story truth. Visual beats are local cut truth. The beat plan is where exact transcript timing decides local location, visible characters, mentioned-only characters, props/UI, visual job, and beat-level reference strategy.
   ...
   - Every beat must carry exact local narration text, Whisper-derived `start_sec`/`end_sec`, `editorial_cues`, `visual_job`, `suggested_shot_job`, `local_location`, `visible_characters`, `mentioned_only_characters`, `local_props`, `local_ui_elements`, beat-level `ref_needs` / `beat_ref_requirements`, and a `location_timeline_label` when applicable. Empty `visual_beat_script_excerpt` / `visual_beat_action` is a production blocker because prompt authoring otherwise falls back to repeated parent-scene summaries.
   - Beat-level `ref_needs` must decide whether the local reference strategy is `standalone_ref`, `no_ref_needed`, `derive_from_first_clean_cut`, `derive_from_first_clean_wide_cut`, or `derive_from_best_cut`. This is local edit direction for reference planning; semantic `ref_requirements` remain broad scoped target coverage.
...
15. Visual reference planning.
   ...
   - Consume both broad semantic scenes and local visual beats. Semantic scenes define broad coverage and scene-level location targets; visual beats define transcript-timed local ref needs and generation-mode strategy. If these disagree, the visual beat excerpt/local cut truth should drive the reference decision, with a warning rather than silent deterministic invention.
