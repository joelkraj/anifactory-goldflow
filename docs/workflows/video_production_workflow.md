...
Registry version: `2026-07-11.5`. This block is generated by `npm run docs:workflow`; edit `scripts/lib/pipeline-stage-registry.mjs`, not this table.
...
23. Directed motion and render.
   - Visual prompt authoring writes one beat-level `shot_manifest.motion_intent`: focal subject, normalized anchors, restrained scale, easing, behavior, and the editorial reason the movement serves the cut. Strong impact, reveal, focus-shift, and UI cuts may also use 2-5 normalized `motion_keyframes` for purposeful hold, move, overshoot, settle, and final-hold phases. Keyframes are selective; calm readable compositions should remain static or use one restrained move. Run `goldflow visual motion-plan` after image QA. The plan is bound to exact accepted image hashes, uses authored intent as its baseline, lets explicit per-cut image-QA focal overrides supersede that baseline after the real frame is inspected, and uses conservative shot-staging fallback only for legacy prompts. Missing focal intent becomes a smooth static hold; production does not use hash-random motion directions.
   - Render writes a per-frame motion trace and blocks unplanned within-segment direction reversals, discontinuities, malformed durations, and stale motion-plan inputs. Intentional keyframed overshoot-and-settle reversals are valid between segments.
   ...
   - `smooth_fast_ken_burns` is the established premium render profile. It uses 60 fps, no heavy oversampling, and the directed motion plan. If its integer `zoompan` crop visibly micro-jitters, use `smooth_subpixel_ken_burns`; that profile applies the same anchors through a cubic per-frame perspective transform for subpixel movement at 1080p/60 fps. Validate a bounded A/B before replacing an approved render. `fill_ken_burns` is an explicit legacy/diagnostic profile.
...
node bin/goldflow.mjs visual motion-proof-plan --episode-dir <episode-dir> --episode ep_01 --scope-end-sec 60 --recipe <recipe.json> --output-dir <episode-dir>/review_samples/editorial_motion_v3
node bin/goldflow.mjs render start --channel <channel> --series <series> --week <week> --episode ep_01 --transition-plan <episode-dir>/review_samples/editorial_motion_v3/transition_edit_plan_ep_01-editorial-v3.json --motion-plan <episode-dir>/review_samples/editorial_motion_v3/motion_edit_plan_ep_01-editorial-v3.json --motion smooth_subpixel_ken_burns --diagnostic-proof true --proof-scope-end-sec 60 --allow-transition-sfx-on-narrator-only true --work-dir <episode-dir>/review_samples/editorial_motion_v3/render-work --motion-trace-output <episode-dir>/review_samples/editorial_motion_v3/motion_trace.jsonl --output <episode-dir>/review_samples/editorial_motion_v3/editorial_motion_v3_first_60.mp4 --report-output <episode-dir>/review_samples/editorial_motion_v3/render_report.json --workflow-bypass true
...
6. Render
   ...
   - Motion style: use `--motion smooth_fast_ken_burns` for the established fast path or `--motion smooth_subpixel_ken_burns` when a bounded proof shows `zoompan` micro-jitter. Both consume the same directed motion plan at 60 fps; the subpixel profile trades some clip-render speed for cubic fractional movement without heavy oversampling. Verify `render_motion.mode`, `render_motion.fps`, cache reuse counts, motion behavior variation, and frame-cadence smoothness in the report. Use `fill_ken_burns` only as a deliberate legacy/diagnostic comparison.
