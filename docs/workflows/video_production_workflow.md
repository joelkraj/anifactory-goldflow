...
Registry version: `2026-07-14.1`. This block is generated by `npm run docs:workflow`; edit `scripts/lib/pipeline-stage-registry.mjs`, not this table.
...
Planner calls use exact content-addressed cache reuse by default. Prompt hash, model, and reasoning effort must match, and malformed cached output is ignored. Use `goldflow run advance` to walk automatic stages without drift; it rereads the ledger after every command and stops at approvals, unapproved spend, render permission, blockers, stale artifacts, risk decisions, manual command shapes, or retry caps.
...
10. TTS generation and stitch.
   - Uses ModelsLab Qwen TTS with `15` synthesis workers by default. Completed units are preserved; request-level retry/backoff handles transient provider throttling without restarting the batch.
...
14. Visual beat planning.
   ...
   - After the first 20 minutes, visual pacing can relax when the story is in stable explanation, dialogue, analysis, or aftermath. The LLM may choose deliberate reuse only from deterministic earlier candidates with the same continuity block, location, visible cast, depiction mode, and active state. Reuse is capped, non-chained, hash/source recorded, and receives independent motion. New locations, states, reveals, public status turns, action, emotional pivots, and cliffhangers remain fresh.
...
21. Image generation.
   ...
   - Codex Imagen means built-in Codex image generation workers coordinated by `goldflow imagegen codex-work`. Create one content-addressed manifest for the exact ref/cut IDs, lease one item per reusable worker, write one correctly named PNG in the leased attempt folder, complete with the exact path and SHA-256, validate the manifest, and only then import with `goldflow imagegen import-staged-codex`. Run a representative four-item verification wave, then keep eight workers full and scale to twelve after a clean wave. Queue status, expired leases, retries, and deadletters are asset-scoped; completed items are never regenerated. Do not use nested `codex exec --enable image_generation` or copy the newest/global Codex cache image as a production route.
   - Codex worker command loop: `imagegen codex-work --action create ... --image-ids <ids>` returns the manifest path; each reusable worker calls `--action lease --manifest <path> --worker-id <id>`, generates from the returned assignment, then calls `--action complete --manifest <path> --asset-id <id> --lease-token <token> --worker-id <id> --source <assigned-output.png> --sha256 <sha>`. The parent polls `--action status`, tops off free workers immediately, runs `--action validate`, and imports with `imagegen import-staged-codex --manifest <path> --image-ids <exact-ids>`. Use `--references-only true --reference-ids <ids>` for reference manifests. A worker failure calls `--action fail`; it never restarts the episode or another completed cut.
...
22. Image focal analysis and output QA.
   - Run `goldflow imagegen analyze` after the complete no-force imagegen report. It analyzes actual accepted raster geometry with a hash-cached local-contrast saliency pass, writes conservative focal anchors, and marks edge/low-information composition exceptions without pretending to identify story semantics.
   - Run `goldflow imagegen qa` after focal analysis and before render.
   ...
   - Default review is exception-driven: inspect the dedicated sheet for the first three minutes, physical actions/contact geometry, dense casts, routed hero/risk cuts, saliency composition exceptions, and an eight-percent deterministic sample of ordinary four-reference integrations. Generate the complete five-minute contact-sheet set only with `--write-full-contact-sheets true` for a diagnostic audit.
...
26. Post-publish feedback.
   - Export YouTube retention as CSV/JSON and run `goldflow analytics ingest --episode-dir <episode-dir> --input <export> --snapshot-label 24h`, then repeat for a later snapshot such as 7d.
   - The report binds each point/drop/rise to the exact beat, image, visual/shot job, location contract, cut duration, ref count, provider, motion behavior, and nearby transition.
   - Aggregate several passed reports with `goldflow analytics aggregate --inputs <report1,report2,...>`. Treat the aggregate as evidence for an operator-reviewed policy change; never tune the pipeline automatically from one episode.
...
node bin/goldflow.mjs tts qwen --channel <channel> --series <series> --week <week> --episode ep_01 --suffix -modelslab-qwen --native-speed 1.25 --concurrency 15
