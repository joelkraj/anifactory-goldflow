...
14. Visual beat planning.
   ...
   - Beat-level `ref_needs` may suggest whether the local reference strategy is `standalone_ref`, `no_ref_needed`, `derive_from_first_clean_cut`, `derive_from_first_clean_wide_cut`, or `derive_from_best_cut`, but they do not lock official reference targets. They are local evidence for reference planning; semantic `ref_requirements` remain broad scoped target coverage, and the visual-reference LLM authors final targets.
...
15. Visual reference planning.
   ...
   - Consume broad semantic scenes, local visual beats, and the director inventory. Semantic scenes define broad coverage and scene-level location targets; visual beats define transcript-timed local facts plus advisory ref hints; the inventory ledger decides asset economy. If these disagree, the visual beat excerpt/local cut truth and inventory director role should drive the reference decision.
   ...
   - Generic groups such as guild masters, guards, police, students, workers, witnesses, or crowds are not character identity refs unless a specific named person recurs. If their look matters, the inventory should route them to a uniform/faction/wardrobe/action-style reference or derive-from-cut strategy.
   ...
   - Derived refs are handled inside image generation, not by rerunning visual planning. A target with `generation_mode: derive_from_first_clean_cut`, `derive_from_first_clean_wide_cut`, or `derive_from_best_cut` remains pending until the scoped seed cut is generated, then `imagegen promote-derived-refs` copies that cut into `assets/images/references` and updates `visual_reference_plan.json` / `character_state_refs.json`.
   - After derived-ref promotion, the hardened prompt manifest remains the production prompt contract. Imagegen attaches the newly available ref path from `shot_manifest` / `reference_requirements` at runtime; do not rerun `visual plan`, `visual review`, or `visual harden` merely to refresh promoted reference paths.
...
18. Visual prompt planning.
   ...
   - Prompt bodies must always include explicit polished 2D anime/manhwa style language in text: clean line art, cel-shaded characters, cinematic webtoon/manhwa lighting, and non-photorealistic painted backgrounds. Do not rely on attached refs alone for style, especially with ModelsLab.
...
20. Visual prompt sanitation.
   ...
   - There is no production deterministic creative repair lane. If a cut needs a different location, visible character, composition, shot job, or ref priority, send it back to `visual review --auto-resolve` or re-plan the cut/scene. Harden may report blockers, but the repair still happens through LLM review auto-resolve, not deterministic prose substitution. If capped harden feedback still looks ambiguous, the pipeline must call for manual agent review rather than forcing another full plan rerun.
...
21. Image generation.
   ...
   - If `run status` reports pending derived refs, generate only the listed seed cut IDs with `imagegen start --seed-derived-refs true --cut-ids <ids>`, then run `imagegen promote-derived-refs`. Promotion writes `derived_reference_promotion_report_<episode>.json`; unresolved entries mean the seed cut is still missing or the reference plan needs manual review.
...
# writes reference_inventory_ledger.json + visual_reference_plan.json + character_state_refs.json
