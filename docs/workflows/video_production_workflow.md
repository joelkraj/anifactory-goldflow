...
12. Audio pace check.
   ...
   - If provider TTS misses the gate, `run status` calculates a bounded native-speed adjustment and routes back to `tts qwen --native-speed <speed> --force true`, followed by fresh Whisper timing and audio pace check.
...
20. Visual prompt sanitation.
   ...
   - Final beat starts are excerpt-aligned, not merely proportional. Snap each exact `visual_beat_script_excerpt` to local Whisper words after editorial splitting. When narration omits a bracketed UI/stat line, interpolate that one beat between neighboring matched excerpts and record the low-confidence/unspoken status; do not let the omitted text shift the remaining scene images several seconds ahead or behind narration.
...
6. Render
   ...
   - Motion style: `--motion smooth_fast_ken_burns --motion-strength 1.75 --render-concurrency 4 --clip-preset veryfast --final-preset veryfast` is the premium default. Verify `render_motion.mode`, `render_motion.fps`, cache reuse counts, and motion behavior variation in the report. Use `fill_ken_burns` only as a deliberate legacy/diagnostic comparison.
