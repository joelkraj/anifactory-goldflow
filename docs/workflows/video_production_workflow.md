...
23. Directed motion and render.
   - Visual prompt authoring writes one beat-level `shot_manifest.motion_intent`: focal subject, normalized start/end anchors, restrained scale, easing, behavior, and the editorial reason the movement serves the cut. Run `goldflow visual motion-plan` after image QA. The plan is bound to exact accepted image hashes, uses authored intent as its baseline, lets explicit per-cut image-QA focal overrides supersede that baseline after the real frame is inspected, and uses conservative shot-staging fallback only for legacy prompts. Missing focal intent becomes a smooth static hold; production does not use hash-random motion directions.
