...
Registry version: `2026-07-12.1`. This block is generated by `npm run docs:workflow`; edit `scripts/lib/pipeline-stage-registry.mjs`, not this table.
...
| 25 | `parallax_asset_generation` | automatic | `parallax_assets_hashes_and_candidate_scope` |
| 26 | `parallax_asset_approval` | operator_or_agent | `parallax_asset_decisions_and_hashes` |
| 27 | `motion_edit_plan` | automatic | `motion_plan_hashes_and_geometry` |
| 28 | `premium_render` | automatic | `render_hashes_and_timeline` |
| 29 | `final_qa` | operator_or_agent | `final_qa_status_and_hashes` |
| 30 | `upload_packaging` | operator | `upload_packaging_source_hash` |

Command order: `run preflight -> ingest source -> script approve -> script pace-check -> script targeted -> semantic plan -> voice plan -> tts qwen -> audio whisper-timing -> audio pace-check -> timing bind -> audio enrich-sfx-score -> audio longform-bed -> visual beats -> visual refs -> visual approve-ref-plan -> [reference_generation] -> visual approve-refs -> visual plan -> visual harden -> visual review -> visual transitions -> imagegen start -> imagegen qa -> visual parallax-assets -> visual approve-parallax -> visual motion-plan -> render start -> final qa -> upload packaging`
