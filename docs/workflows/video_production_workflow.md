...
Registry version: `2026-07-11.4`. This block is generated by `npm run docs:workflow`; edit `scripts/lib/pipeline-stage-registry.mjs`, not this table.
...
| 17 | `reference_generation` | automatic | `reference_candidate_selection_and_hashes` |
...
17. Reference generation and approval.
   ...
   - For ModelsLab Flux Klein, run `goldflow visual cast-refs`. Generate isolated candidate sets concurrently: four for core character states used in at least 20 beats, three for major/required character states, two for recurring lower-risk characters or key non-character anchors, and one for low-risk refs.
   - Give the actual candidate rasters, selected target contract, exact story evidence, and active-state evidence to a vision-capable Codex casting director. Selection requires passing story accuracy, conditioning cleanliness, visual appeal, and anime/manhwa style scores. If no candidate passes, the reviewer authors one complete corrected prompt and reruns only that reference, up to two iterations by default.
   - Candidate generation and selection reports merge prior successful refs so scoped retries are resumable. Promotion atomically replaces the official path, archives the prior official image, and records selected/promoted SHA-256 hashes.
   - Agent/operator inspect promoted core cast refs before any scene cut generation.
...
node bin/goldflow.mjs visual cast-refs --channel <channel> --series <series> --week <week> --episode ep_01 --reference-image-model flux-klein --auto-resolve true --max-resolve-iterations 2
