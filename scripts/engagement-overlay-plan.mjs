...
async function callCodex(prompt, stageName) {
  ...
  codexArgs.push("-c", `model_reasoning_effort="${reasoningEffort}"`, "-c", 'model_verbosity="medium"', "-o", codexOutputPath);
  const output = await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Codex engagement plan ${stageName} timed out`));
    }, Number(process.env.ANIFACTORY_ENGAGEMENT_PLANNER_CODEX_TIMEOUT_MS ?? 600_000));
    ...
    child.on("exit", async (code) => {
      clearTimeout(timer);
      if (code !== 0) {
        reject(new Error(`Codex engagement plan ${stageName} exited ${code}: ${stderr || stdout}`));
        return;
      }
      const text = await fs.readFile(codexOutputPath, "utf8").catch(() => stdout || stderr);
      resolve(text);
    });
    child.stdin.end(prompt);
  });
  return { provider: "codex", model: model || "codex_cli_default", prompt_path: promptPath, output_path: codexOutputPath, parsed: extractJson(output) };
}
